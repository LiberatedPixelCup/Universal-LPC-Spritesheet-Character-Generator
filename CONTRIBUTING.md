...
UI components are Mithril **`m.Component<Attrs, State>`** objects. Thread **`catalog: CatalogReader`** and **`state: State`** to composition boundaries, then prefer render-ready models with narrow commands for leaf components. The filter controls and `CurrentSelections` demonstrate this: `main.ts` builds the application model graph, while `App` and `FiltersPanel` only forward lazy constructors for the relevant slices; each leaf invokes its constructor from `view`. Do not read a hidden global catalog or a module-level `state`.
