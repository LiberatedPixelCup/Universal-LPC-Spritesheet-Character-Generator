...
UI components are Mithril **`m.Component<Attrs, State>`** objects. Thread **`catalog: CatalogReader`** and **`state: State`** to composition boundaries, then prefer render-ready models with narrow commands for leaf components. The filter controls, `CurrentSelections`, Download, and Credits demonstrate this: `main.ts` builds the application model graph, while composition components only forward lazy constructors for the relevant slices. Each rendered leaf invokes its constructor from `view`; a component that owns a `CollapsibleSection` uses an internal content leaf so collapsed content does not build its model. Return Promises from asynchronous Mithril event handlers so Mithril redraws when they settle. Do not read a hidden global catalog or a module-level `state`.
