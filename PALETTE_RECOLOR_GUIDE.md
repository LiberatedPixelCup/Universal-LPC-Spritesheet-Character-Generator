...
(or `npm run build` once). **`npm run validate-site-sources`** updates **CREDITS.csv** and z-positions but does not write `dist/item-metadata.js`. After **`npm run dev`**, open **`dist/item-metadata.js`** to inspect the catalog with readable indentation; after **`npm run build`**, the same file is compact (no pretty-printed JSON).
